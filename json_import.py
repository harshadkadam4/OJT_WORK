...
conn = sqlite3.connect('cricket_db.db')
cursor = conn.cursor()

with open('ipl_data.json') as f:
    data = json.load(f)
...
cursor.execute('''
                   INSERT INTO match(match_type,season,teams_involve,match_no)
                   VALUES (?,?,?,?)  
                   ''',(match_type,season,team_type,match_number,))

cursor.execute('''
        SELECT match_id
        FROM match
        WHERE match_type = ? AND season = ? AND teams_involve = ? AND match_no = ?
        ''', (match_type, season, team_type, match_number))
...
ofc_types = ['match_referees', 'reserve_umpires', 'tv_umpires', 'umpires']
...
by_runs = data['info']['outcome']['by']['runs']
...
cursor.execute('''
     ...
     ''',(match_id,team_a,team_b,target,winner,by_runs,toss_win,decision,player_of_match,venue_id,date,))
...
#bat_team_id_2 = data['innings'][0]['team']
#ball_team_id_2 = data['innings'][1]['team']
